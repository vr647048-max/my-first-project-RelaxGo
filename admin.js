...
function card(b){
  ...
  return `<article class="booking"><div class="booking-top"><div><div class="booking-id">${esc(bookingId)}</div><h2>${esc(b.customer_name)}</h2><div>${esc(b.service)} • ₹${Number(b.price||0).toLocaleString("en-IN")} • ${String(b.payment_method||"online")==="cash"?"💵 Cash on Service":"💳 Online"} • ${paymentLabel} • Visit: ${esc(b.visit_status||current)}</div></div><span class="status">${esc(current)}</span></div>
...
function chatMessageHtml(m){const who=m.sender_role==='customer'?'Customer':'You';return '<div class="chat-msg '+(m.sender_role==='customer'?'customer':'provider')+'"><b>'+esc(who)+'</b><div>'+esc(m.message)+'</div></div>';}
async function loadChat(){if(!chatBookingId)return;const box=document.getElementById('chatMessages');if(!box)return;const result=await sb.from('booking_messages').select('*').eq('booking_id',chatBookingId).order('created_at',{ascending:true});if(result.error){box.innerHTML='<div class="empty">Chat unavailable: '+esc(result.error.message)+'</div>';return;}box.innerHTML=(result.data||[]).map(chatMessageHtml).join('')||'<div class="empty">No messages yet.</div>';box.scrollTop=box.scrollHeight;}
async function openChat(id){chatBookingId=String(id);const b=bookings.find(x=>String(x.id)===chatBookingId);const panel=document.getElementById('chatPanel');if(!panel){alert('Chat panel is not available.');return;}panel.classList.remove('hidden');document.getElementById('chatTitle').textContent='Chat • '+((b&&b.customer_name)||'Customer');await loadChat();if(chatChannel)sb.removeChannel(chatChannel);chatChannel=sb.channel('booking-chat-'+chatBookingId).on('postgres_changes',{event:'INSERT',schema:'public',table:'booking_messages',filter:'booking_id=eq.'+chatBookingId},payload=>{const box=document.getElementById('chatMessages');if(box){box.insertAdjacentHTML('beforeend',chatMessageHtml(payload.new));box.scrollTop=box.scrollHeight;}}).subscribe();}
function closeChat(){if(chatChannel)sb.removeChannel(chatChannel);chatChannel=null;chatBookingId=null;const p=document.getElementById('chatPanel');if(p)p.classList.add('hidden');}
